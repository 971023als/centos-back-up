...
# 변경 사항 확인
grep -q "auth required /lib/security/pam_tally.so deny=5 unlock_time=120 no_magic_root" /etc/pam.d/system-auth
if [ $? -eq 0 ]; then
    OK "/etc/pam.d/system-auth 파일에 성공적으로 추가되었습니다"
else
    WARN "/etc/pam.d/system-auth 파일에 줄을 추가하는 동안 오류가 발생했습니다."
fi
