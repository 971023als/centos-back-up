...
echo "이 시스템에 오신 것을 환영합니다!" | sudo tee -a /etc/issue
echo "다음 경고에 주의하십시오.: " | sudo tee -a /etc/issue

# Edit the /etc/vsftpd/vsftpd.conf file
sudo sed -i 's/#ftpd_banner=.*/ftpd_banner=이 시스템에 오신 것을 환영합니다!/' /etc/vsftpd/vsftpd.conf
sudo echo "ftpd_banner=다음 경고에 유의하십시오.: " | sudo tee -a /etc/vsftpd/vsftpd.conf
